...
constexpr const char* kToolVersion = "0.1.3-alpha";
constexpr const char* kLogName = "FNVGameNativePatcher.log";
...
const char* patcher_architecture_impl() {
#if defined(_WIN64)
    return "x86-64 (AMD64)";
#elif defined(_WIN32)
    return "x86 (Intel 80386)";
#elif INTPTR_MAX == INT64_MAX
    return "native 64-bit test host";
#else
    return "native 32-bit test host";
#endif
}

fs::path process_executable_path_impl(const char* argv0) {
#ifdef _WIN32
    ...
    for (;;) {
        ...
        if (length == 0) {
            throw Error("GetModuleFileNameW failed while resolving the patcher executable path.");
        }
        if (length < capacity) {
            return fs::path(std::wstring(buffer.data(), length));
        }
        if (buffer.size() >= 32768) {
            throw Error("The patcher executable path exceeds the supported Windows path length.");
        }
        buffer.resize(std::min<std::size_t>(buffer.size() * 2, 32768));
    }
#else
    std::error_code ec;
    const fs::path proc_path = fs::read_symlink("/proc/self/exe", ec);
    ...
    throw Error("Unable to resolve the patcher executable path.");
#endif
}

std::uint16_t read_u16(const std::vector<std::uint8_t>& data, std::size_t offset) {
    if (offset > data.size() || data.size() - offset < 2) throw Error("Unexpected end of file while reading a 16-bit value.");
...
std::uint32_t read_u32(const std::vector<std::uint8_t>& data, std::size_t offset) {
    if (offset > data.size() || data.size() - offset < 4) throw Error("Unexpected end of file while reading a 32-bit value.");
...
void write_u16(std::vector<std::uint8_t>& data, std::size_t offset, std::uint16_t value) {
    if (offset > data.size() || data.size() - offset < 2) throw Error("Unexpected end of file while writing a 16-bit value.");
...
void write_u32(std::vector<std::uint8_t>& data, std::size_t offset, std::uint32_t value) {
    if (offset > data.size() || data.size() - offset < 4) throw Error("Unexpected end of file while writing a 32-bit value.");
    data[offset] = static_cast<std::uint8_t>(value & 0xff);
    data[offset + 1] = static_cast<std::uint8_t>((value >> 8) & 0xff);
...
void print_inspection_report(
    ...
    if (inspection.authenticode.offset != 0 || inspection.authenticode.size != 0) {
        output << "  Authenticode file offset: 0x" << std::hex << inspection.authenticode.offset
                  << "\n  Authenticode size: 0x" << inspection.authenticode.size << std::dec << "\n";
    }
    output << "  Large Address Aware: " << (inspection.large_address_aware ? "yes" : "no") << "\n"
...
void print_cache_coverage(const fs::path& directory, std::ostream& output) {
    ...
    if (primary_patched && cache_patched) {
        output << "  Recommended next action: Launch FalloutNV.exe normally through GameNative.\n";
    } else if (!fs::exists(cache)) {
        output << "  Recommended next action: Make GameNative rerun Unpack Files, then close the game and rerun this patcher.\n";
    } else {
        output << "  Recommended next action: Run the patcher to synchronize both executable copies.\n";
    }
}

void patch(const fs::path& directory, std::ostream& output) {
    const fs::path primary = directory / kTargetName;
    const fs::path cache = directory / kCacheTargetName;
    if (!fs::exists(primary)) {
        throw Error(
            "Status: FalloutNV.exe was not found beside the patcher.\n"
            "No files were changed.\n"
    ...
    if (!any_write) {
        for (auto it = plans.rbegin(); it != plans.rend(); ++it) {
            print_inspection_report(it->inspection, directory, output, it->file.name.c_str());
        }
        output << "No files were changed. Both GameNative launch copies are already patched.\n";
        print_cache_coverage(directory, output);
        return;
    }
    ...
    output << "Patched GameNative's Fallout: New Vegas executable pair successfully.\n";
    for (const auto& plan : plans) {
        ...
        if (plan.needs_write && plan.inspection.authenticode.state == AuthenticodeState::StaleOutOfBounds) {
            output << "    Stale GameNative Authenticode metadata cleared\n";
        }
        if (plan.needs_write) {
            output << "    Original saved as " << plan.file.backup.filename().string() << "\n";
        }
    }
    output << "  Large Address Aware enabled\n"
...
void restore(const fs::path& directory, std::ostream& output) {
    ...
    for (const auto& plan : plans) {
        if (plan.needs_restore) {
            output << "Restored " << plan.file.name << " from "
...
void print_single_verification(
    ...
    if (!fs::exists(file.target)) {
        output << "  Condition: file not found\n"
                  << "  Patchable: no\n"
                  << "  Files changed: no\n";
        return;
    }
    ...
    try {
        ...
        print_inspection_report(inspection, directory, output, "  Executable report");
        output << "  Files changed: no\n";
    } catch (const std::exception& error) {
        output << "  Condition: malformed or unsupported PE data\n"
                  << "  Details: " << error.what() << "\n"
                  << "  Patchable: no\n"
                  << "  Files changed: no\n";
    }
...
void verify(const fs::path& directory, std::ostream& output) {
    output << "Verification report\n";
    const auto files = managed_executables(directory);
    // Human-facing order: normal launch target first, then GameNative cache.
    print_single_verification(directory, files[1], output);
    print_single_verification(directory, files[0], output);
    print_cache_coverage(directory, output);
}

void print_help_impl(std::ostream& output) {
    output << kToolName << " " << kToolVersion << "\n\n"
...
const char* patcher_architecture() {
    return patcher_architecture_impl();
}

fs::path process_executable_path(const char* argv0) {
    return process_executable_path_impl(argv0);
}

void print_help(std::ostream& output) {
    print_help_impl(output);
}
...
void run_operation(
    ...
    if (command == "--patch") {
        patch(directory, output);
    } else if (command == "--verify") {
        verify(directory, output);
    } else if (command == "--restore") {
        restore(directory, output);
    } else if (command == "--help" || command == "-h") {
        print_help_impl(output);
    } else {
        throw Error("Unknown argument: " + command);
    }
}
